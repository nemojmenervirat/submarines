...
import com.nemojmenervirat.submarines.logic.SubmarineFactory.Type;
...
public class Game {
	...
	private void resetSubmarines(List<Submarine> submarines) {
		for (Submarine submarine : submarines) {
			...
			submarine.setCurrentAttacks(submarine.getTotalAttacks());
	...
	public Game(User leftUser, User rightUser, GameType gameType) {
...

		{
			...
			Submarine sub1 = SubmarineFactory.createNew(Type.T1, Direction.LTR, leftLine - RandomXRange / 2 + randomX,
					playgroundHeight / 6 * 1 - RandomYRange / 2 + randomY);
			randomY = random.nextInt(RandomYRange);
			randomX = random.nextInt(RandomXRange);
			Submarine sub2 = SubmarineFactory.createNew(Type.T2, Direction.LTR, leftLine - RandomXRange / 2 + randomX,
					playgroundHeight / 6 * 2 - RandomYRange / 2 + randomY);
			randomY = random.nextInt(RandomYRange);
			randomX = random.nextInt(RandomXRange);
			Submarine sub3 = SubmarineFactory.createNew(Type.T3, Direction.LTR, leftLine - RandomXRange / 2 + randomX,
					playgroundHeight / 6 * 3 - RandomYRange / 2 + randomY);
			randomY = random.nextInt(RandomYRange);
			randomX = random.nextInt(RandomXRange);
			Submarine sub4 = SubmarineFactory.createNew(Type.T4, Direction.LTR, leftLine - RandomXRange / 2 + randomX,
					playgroundHeight / 6 * 4 - RandomYRange / 2 + randomY);
			randomY = random.nextInt(RandomYRange);
			randomX = random.nextInt(RandomXRange);
			Submarine sub5 = SubmarineFactory.createNew(Type.T5, Direction.LTR, leftLine - RandomXRange / 2 + randomX,
	...
	public boolean canMove(User user, Submarine submarine, Move move) {
		...
		Rectangle playgroundBounds = new Rectangle(direction == Direction.LTR ? 0 : playgroundWidth / 2, 0,
				playgroundWidth / 2, playgroundHeight);
		int xMove = 0;
		int yMove = 0;
		switch (move) {
		case LEFT:
		...
		Rectangle submarineBounds = new Rectangle((int) (submarine.getStartX() + submarine.getX()) + xMove,
	...
	public Submarine getHitTarget(User user, Submarine attacker) {
		...
		List<Submarine> possibleTargets = user.equals(leftUser) ? rightUserSubmarines : leftUserSubmarines;
